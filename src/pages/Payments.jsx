...
// Tracks all student payments — fees collected, pending, overdue
// Same code style as Students.jsx and Visitors.jsx

import { useState, useEffect } from 'react'
import { supabase } from '../supabase'
import theme from '../theme'

// payment type options for the filter dropdown
const PAYMENT_TYPES = [
  'All Types',
  'Consultation Fee',
  'Application Fee',
...
export default function Payments() {
  ...
  // modal state for adding a new payment
  ...
  // load all payments when page opens
  useEffect(() => {
    loadPayments()
  }, [])
  ...
  // stat cards at the top — same pattern as Visitors.jsx
  const stats = [
    {
  ...
  // filter the list by search + type + status
  const filtered = payments.filter(p => {
    const matchSearch = p.student_name?.toLowerCase()
  ...
  async function handleAddPayment(e) {
    ...
    const { error } = await supabase.from('payments').insert({
      ...
      method:       form.method,
      note:         form.note,
      status:       'pending',   // starts as pending until confirmed
    })
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{
          display: 'grid',
          gridTemplateColumns: '2fr 1.5fr 1.5fr 1fr 1.2fr 1fr 1.5fr',
          padding: '10px 16px',
        ...
        {filtered.map((p, i) => (
          ...
          >
            ...
            {/* payment type e.g. Visa Fee */}
            <div style={{ fontSize: 13, color: theme.textMid }}>
              {p.type || '—'}
            </div>

            {/* payment method e.g. Cash, eSewa */}
            <div style={{
              fontSize: 12,
              color: theme.textLight,
              display: 'flex',
            ...
            <div>
              <span style={{
                ...
                fontSize: 11,
                fontWeight: 600,
                background: badgeStyle(p.status).bg,
                color: badgeStyle(p.status).color,
            ...
            <form onSubmit={handleAddPayment}>
              ...
              <Field label="Payment Type">
                <select
                  ...
                  style={inputStyle(theme)}
                >
                  {PAYMENT_TYPES.filter(t => t !== 'All Types').map(t => (
                    <option key={t}>{t}</option>
                  ))}
                </select>
              </Field>
  ...
  // mark a payment as paid — updates just the status field
...
// form field with label above — reused inside modal
...
// shared style for all modal inputs — avoids repeating
